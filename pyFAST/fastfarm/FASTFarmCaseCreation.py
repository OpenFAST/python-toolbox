...
class FFCaseCreation:
            ...
            for case in range(self.nCases):
                ...
                for seed in range(self.nSeeds):
                    ...
                    if self.inflowStr == 'MannBox':
                        turbsimPath = os.path.join(seedPath, 'MannBox')
                ...
                if self.inflowStr =='MannBox':
                    ...
                    if writeFiles:
                        for seed in range(self.nSeeds):
                            ...
                            Mannfile=os.path.join(currPath,f'Seed_{seed}','MannBox',f'Vhub{Vhub_}_TI{tivalue_}_PL{shear_}')
                            self._getMBBoxesParamsForInflow(Mannfile, Vhub_, HubHt_, shear_)
                            self.InflowWindFile['FileName_u']   = f"./MannBox/Vhub{Vhub_}_TI{tivalue_}_PL{shear_}_u.bin"
                            self.InflowWindFile['FileName_v']   = f"./MannBox/Vhub{Vhub_}_TI{tivalue_}_PL{shear_}_v.bin"
                            self.InflowWindFile['FileName_w']   = f"./MannBox/Vhub{Vhub_}_TI{tivalue_}_PL{shear_}_w.bin"
        ...
        for cond in range(self.nConditions):
            for case in range(self.nHighBoxCases):
                ...
                if self.verbose>3:
                    print(f'Generating high-res box setup for cond {cond} ({self.condDirList[cond]}), case {case} ({self.caseDirList[case]}).')
                for seed in range(self.nSeeds):
                    seedPath = os.path.join(self.path, self.condDirList[cond], self.caseDirList[case], f'Seed_{seed}', 'MannBox')
                    ...
                    currentMannFile =  f'Vhub{Vhub_}_TI{tivalue_}_PL{shear_}'
                    # Create and write new Low.inp files creating the proper box with proper resolution
                    # pdb.set_trace()
        ...
        for cond in range(self.nConditions):
            for case in range(self.nCases):
                for seed in range(self.nSeeds):
                    ...
                    MBfilePath = os.path.join(self.path, self.condDirList[cond], self.caseDirList[case], f'Seed_{seed}', 'MannBox')
                    MannFile =  f'Vhub{Vhub_}_TI{tivalue_}_PL{shear_}'
    ...
    ######## Farm Inputs
        ...
        extent_wake=2*extent_low[1]
        hubHeight=HubHt
        HubWS=Vhub
        Z0_Box      = self.zbot
        
        xWT=xWT+min(xWT)+extent_X*0.5
