...
def main():
    ...
    ax.plot(dfRad_AD['r/R_[-]'].values, dfRad_AD['B1Cl_[-]'].values / dfRad_AD['B1Cl_[-]'].max(),
            label='Lift coefficient (AeroDyn)')
    ax.plot(dfRad_ED['r/R_[-]'].values, dfRad_ED['B1TDx_[m]'].values / dfRad_ED['B1TDx_[m]'].max(),
            label='Flap displacement (ElastoDyn)')
    ax.set_xlabel('r/R [-]')
